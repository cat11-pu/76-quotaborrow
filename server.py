...
ENGINE = Allocator()
ROUTES = {
    "/request": lambda payload: ENGINE.request(payload["tenant"], payload["amount"]),
    "/release": lambda payload: ENGINE.release(payload["tenant"], payload["amount"]),
    "/reclaim": lambda payload: ENGINE.reclaim(payload["tenant"]),
